        ...
        implements VerleihService
{
            ...
            throws ProtokollierException
    {
        ...
        removeVormerkerBeiAusleihe(kunde, medien); // entfernt den ersten Vormerker
    ...
    {
        assert kunde != null : "Vorbedingung medien verletzt: null";
        assert medien != null : "Vorbedingung kunde verletzt: null";

        if (istVormerkenMoeglich(kunde, medien))
        {
            for (Medium medium : medien)
            {
                if (!_vormerkkarten.containsKey(medium)) //prüft ob für das Medium eine Vormerkkarte existiert.
                {
                ...
                Vormerkkarte karte = _vormerkkarten.get(medium);
                karte.fuegeVormerkerHinzu(kunde);
            }
            informiereUeberAenderung();
        }
    }
    ...
    public boolean istVormerkenMoeglich(Kunde kunde, List<Medium> medien)
    {
        ...
        assert medien != null : "Vorbedingung medien verletzt: null";
        ...
        for (Medium medium : medien)
        {
            ...
            {
                Vormerkkarte karte = _vormerkkarten.get(medium);
                if (!karte.istVormerkenMöglich(kunde))
                {
                    return false;
                }
            }
    ...
    {
        assert position >= 0 : "Vorbedingung position >= 0 verletzt";
        assert medium != null : "Vorbedingung medium verletzt: null";

        if (_vormerkkarten.containsKey(medium))
        {
            return _vormerkkarten.get(medium)
                .getVormerker(position);
        }
    ...
    @Override
    public void removeVormerkerBeiAusleihe(Kunde kunde, List<Medium> medien)
    {
        assert kunde != null : "Vorbedingung kunde verletzt: null";
        assert medien != null : "Vorbedingung medien verletzt: null";
